...
### Setting up source tree for stuff I need

To start with, I need to create a new app under which I do this development. So I say:
...
I want to use STM32F3discovery board for this, so I need the board support package for that one. I know where it, so I fetch the package list from that repository, and install the egg for the BSP.

```no-highlight
[marko@IsMyLaptop:~/src/air_quality]$ newt app add-pkg-list mynewt_stm32f3 https://github.com/runtimeinc/mynewt_stm32f3.git
...
[marko@IsMyLaptop:~/src/air_quality]$ newt pkg install hw/bsp/stm32f3discovery
...
Ooops. I forgot to bring in the package list for larva itself. That's the one with most of the packages. Including libs/cmsis-core.

I want the latest and greatest from larva, so I want to bring in the 'develop' branch. Note that normally stuff use of master branch is recommended. In that case you would drop the '-b develop' from command line.

However, I want to bring in the packages from develop branch.
...
That's better. I want to make sure I have all the needed bits for supporting my board, so I want to build blinky project for it first.
...
And then create a target for it and build it.
...
Images need to be smaller for this board, so I'm going to switch over to using baselibc as my libc.

To do that, I modify the project's egg file project/blinky/pkg.yml. And add *libs/baselibc* to list of package dependencies (pkg.deps).
...
I know that this platform uses bootloader, so I better create a target for that too. And download the bootloader package.
...
Next I must download the targets to board, and see that the LED actually blinks. I plug in the STM32F3 discovery board to my laptop, and then I say:
...
Now that I have my system setup, I can start creating my own stuff.
First I want to create a project for myself, I'll start by getting project template from blinky, as it pretty much has what I want.
...
The I modify the pkg.yml for air_quality in order to change the *pkg.name* to be *project/air_quality*.
...
One of the sensors I want to enable is SenseAir K30, which will connect to my board over serial port.
To start development of the driver, I first need to create a package description for it, and add stubs for sources.

So I add few files. pkg.yml to describe the driver, and then header stub followed by source stub.
...
And add a call to my main() to initialize this driver.
```no-highlight
...
While developing the driver, I want to issue operations from console asking it to do stuff. The way to do this is to register my command handler with shell. Whenever my custom command is issued, I can respond to it.

The way I do is first adding a dependency to shell package for my senseair driver. So I change libs/my_drivers/senseair/pkg.yml to have the following:
```no-highlight
pkg.name: libs/my_drivers/senseair
pkg.deps:
    - hw/hal
...
And then register my shell command in *senseair_init()*.
...
Then I build this, download to target, and start minicom on my console port.
...
Now that's great. I can connect the hardware to board and start developing code for the driver itself.

### Use of HAL for drivers

The sensor has a serial port connection, and that's how I'm going to connect to it. My original BSP, hw/bsp/stm32f3discovery, has only one UART set up (as specified in src/hal_bsp.c, include/hal/bsp.h). Therefore, I need to create my own bsp which has configuration for this added hardware.

So in the shell I make a copy of the original BSP, and then change the package file a little.
```no-highlight
[marko@IsMyLaptop:~/src/air_quality]$ cp -R hw/bsp/stm32f3discovery hw/bsp/stm32f3discovery_with_senseair
```

Then I modify the pkg.yml in the copied BSP to assign name for this package.
...
And I want to use this BSP with my target. So I change the BSP in the target definition.
...
I add the 2nd serial port to my new BSP.

I modify the include/hal/bsp.h to increase UART_CNT to 2, and add a definition of the 2nd logical UART. I will use this in my sensor driver.
...
With this in place, I can refer to serial port where my SenseAir sensor is by a logical number. So this makes the code more platform independent; I might want to connect this sensor to another board, like Olimex. I'll also use the HAL UART abstraction to do the UART port setup and data transfer. That way I don't need to have any platform dependent pieces within my little driver.

I'll show what the driver code ended up looking like. Here's my header file, filled in from stub I created earlier.
...
As you can see, I added logical UART number to my init routine. And added 'read' function, which is a blocking read. If I was making this a real product, I would probably have a callback for reporting the results.
...
And I modified my main() for senseair driver init.
...
You can see from the code that I'm using the HAL interface to open a UART port, and use OS semaphore as a way of blocking the task when waiting for read response to come back from the sensor.
